	...
	: Super(ObjectInitializer.SetDefaultSubobjectClass<USoldierMovementComponent>(ASoldier::CharacterMovementComponentName))
{
	...
	static ConstructorHelpers::FObjectFinder<USkeletalMesh> NewMesh(TEXT("SkeletalMesh'/Game/FPSTemplate/Soldier/Animations2/erika_archer.erika_archer'"));
...
void ASoldier::GetLifetimeReplicatedProps(TArray<FLifetimeProperty> & OutLifetimeProps) const
{
	...
	DOREPLIFETIME(ASoldier, cshield);
	DOREPLIFETIME(ASoldier, powerupisactive);
	DOREPLIFETIME(ASoldier, rshield);
	DOREPLIFETIME(ASoldier, rfarrow);
	DOREPLIFETIME(ASoldier, scollected);
	DOREPLIFETIME(ASoldier, swork);
	DOREPLIFETIME(ASoldier, crossaim);
...
void ASoldier::Tick(float DeltaTime)
{
	...
	if (rshield == 1)
	{
		rsTimer = rsTimer + DeltaTime;
	}

	if (rfarrow == 1)
	{
		rfaTimer = rfaTimer + DeltaTime;
	}

	if (cshield == 1)
	{
		sTimer = sTimer + DeltaTime;
		//rsTimer = rsTimer + DeltaTime;
	}
	if (sTimer >= 20.f)
	{
	...
	if (rsTimer >= 60.f)
	{
		for (TObjectIterator<AShield> Itr; Itr; ++Itr)
		{
			if (Itr->IsA(AShield::StaticClass()))
			{
				AShield* shield = *Itr;
	...
	if (powerupisactive == 1 && scollected == 0 && swork == 1)
	{
		Health = Health - 0.1f;
		faTimer = faTimer + DeltaTime;
		//rfaTimer = rfaTimer + DeltaTime;
	}
	...
	if (rfaTimer >= 90.f)
	{
		for (TObjectIterator<AFireArrow> Itr; Itr; ++Itr)
		{
			if (Itr->IsA(AFireArrow::StaticClass()))
			{
				AFireArrow* firearrow = *Itr;
				firearrow->SetActorHiddenInGame(false);
				firearrow->SetActorEnableCollision(true);
			}
		}
		rfaTimer = 0;
		rfarrow = 0;
	...
	getHealth();
	//GEngine->AddOnScreenDebugMessage(-1, 15.0f, FColor::Green, FString::Printf(TEXT("My health is: %f"), Health));
...
void ASoldier::FireProjectile(TSubclassOf<ABaseProjectile> ProjectileClass)
{
	FActorSpawnParameters SpawnParams;
	SpawnParams.Instigator = this;
...
void ASoldier::Healthplus()
{
	Health = Health + 10.f;
}

float ASoldier::getHealth()
{
	return Health;
	GEngine->AddOnScreenDebugMessage(-1, 15.0f, FColor::Green, FString::Printf(TEXT("My health is: %f"), Health));
}
...
void ASoldier::getShield()
{
	cshield = 1;
}

void ASoldier::Collected()
{
	scollected = 1;
}

void ASoldier::respawnshield()
{
	rshield = 1;
}

void ASoldier::respawnfarrow()
{
	rfarrow = 1;
}

void ASoldier::activepowerup()
{
	powerupisactive = 1;
}

int ASoldier::HasCollected()
{
	return scollected;
}

void ASoldier::Worked()
{
...
int ASoldier::getimpulse()
{
	return impulse;
}

int ASoldier::getcrossaim()
{
	return crossaim;
}